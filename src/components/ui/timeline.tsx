    ...
    <div className="w-full" ref={containerRef}>
      <div ref={ref} className="relative mx-auto max-w-5xl pb-20">
        {data.map((item, index) => (
          <div key={index} className="flex justify-start pt-10 md:gap-10 md:pt-32">
            <div className="sticky top-40 z-40 flex max-w-xs flex-col items-center self-start md:w-full md:flex-row lg:max-w-sm">
              ...
              <h3 className="hidden text-xl font-bold text-neutral-500 md:block md:pl-20 md:text-4xl">
                {item.title}
              </h3>
            </div>
        ...
        <div
          ...
          className="absolute left-8 top-0 w-[2px] overflow-hidden bg-[linear-gradient(to_bottom,var(--tw-gradient-stops))] from-transparent from-0% via-neutral-800 to-transparent to-100% md:left-8"
        >
          <motion.div
            style={{ height: heightTransform, opacity: opacityTransform }}
            className="absolute inset-x-0 top-0 w-full rounded-full bg-gradient-to-t from-red-500 via-purple-500 to-transparent"
          />
        </div>
      </div>
    </div>
